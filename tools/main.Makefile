...
install:				test debug release
	@$(MAKE) FILEDIR=$(FILEDIR) DISBALE_CONTROL_CODES=$(DISBALE_CONTROL_CODES) INSTALL_ACTIVE=YES		ActionInstall
uninstall:
	@$(MAKE) FILEDIR=$(FILEDIR) DISBALE_CONTROL_CODES=$(DISBALE_CONTROL_CODES) INSTALL_ACTIVE=YES		ActionUInstall
build:					test debug release
	@$(MAKE) FILEDIR=$(FILEDIR) DISBALE_CONTROL_CODES=$(DISBALE_CONTROL_CODES) INSTALL_ACTIVE=NO		ActionInstall
	@$(MAKE) done
release-only:			release
	@$(MAKE) FILEDIR=$(FILEDIR) DISBALE_CONTROL_CODES=$(DISBALE_CONTROL_CODES) INSTALL_ACTIVE=NO		ActionDoInstallHead ActionDoInstallRelease
veryclean:				clean
	@$(MAKE) FILEDIR=$(FILEDIR) DISBALE_CONTROL_CODES=$(DISBALE_CONTROL_CODES) INSTALL_ACTIVE=NO		ActionUInstall
debug:					makeDep
	@$(MAKE) FILEDIR=$(FILEDIR) DISBALE_CONTROL_CODES=$(DISBALE_CONTROL_CODES) TARGET_MODE=debug		item
release:				makeDep
	@$(MAKE) FILEDIR=$(FILEDIR) DISBALE_CONTROL_CODES=$(DISBALE_CONTROL_CODES) TARGET_MODE=release	item
lint:					doLint
...
build-honly-tail:
	...
	@echo "HEADER_ONLY=1 THORSANVIL_ROOT=\"$(THORSANVIL_ROOT)\" CXXFLAGS=\"-I$(PREFIX)\" LDLIBS_FILTER=\"$(patsubst $(PREFIX)/%,%,$(wildcard $(PREFIX)/*))\" $(MAKE) FILEDIR=$(FILEDIR) DISBALE_CONTROL_CODES=$(DISBALE_CONTROL_CODES) test"
	@echo
	@echo
	@CWD="$$(pwd)";	\
...
_start:
	...
	@mkdir -p $(META)
	@rm -f $(META)/pipe
ifeq ($(DISBALE_CONTROL_CODES),TRUE)
	@echo 0 > $(META)/pid
else
	@mkfifo $(META)/pipe
...
Print_On:
	@$(ECHO) "DEP:					$(DEP)"
	@$(ECHO) "DISBALE_CONTROL_CODES:$(DISBALE_CONTROL_CODES)"
